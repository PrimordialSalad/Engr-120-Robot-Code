...
/*	I removed the function calls to monitor light front, left, down, and right because I didn't write those functions and I was not sure
	if I would have been allowed to submit them to git.  Therefore they were removed.  Everything else was my own creation.  The robot was made using
	VEX Robot C which is why it looks similar to C but there are a few extra keywords.  This code still has issues, if it is looking for the target and it loses it
	will not find it again, for whatever reason.  It also gets slightly confused if there is copious amounts of sunlight, as it might mess up the IR phototransistor
	readings but most of the time it works.
*/

////////////////////////////////////////////////////End of Header Comment//////////////////////////////////////////////////////////////////////////////////////////

//Right going 60 and left is going 100 will help the robot go straight.
//minLevelFrontSensor < SensorValue[IRsensorFront] Old if condition, in case the new one breaks.  Revert to this one if it does.
...
bool correctDistanceForAttachment() //This checks whether or not the robot is at the correct distance for attachment to the target object.  Signals true or false.
{
	bool result = true;
...
void fixWallCollisionWithoutSonar() //This function checks to see what sensors are being activated by the wall and acts accordingly.
{
	stopMoving();
...
bool moveForwardOnceFound() //This is a function that moves the robot forward once the beacon is found.
{
	...
	while(!correctDistanceForAttachment()) //Continue to drive forward until at the right distance.
	{
		...
		{
			if(detectWallCollisionWithoutSonar() > 0) // If the beacon is still visible out the front but robot is against a wall.  Will remedy the situation accordingly.
			{
		...
		else if(returnDistanceFromFront() > 20 || returnDistanceFromFront() == -1) //Go until this distance.  Perfect distance for connection.
		{
			moveStraightForward();
		}
...
void huntBeacon() //This function is run to allow the robot to locate the target object.
{
	...
	beaconVisibleFront = monitorLightFront(); //This and the rest of the assignment statements are to monitor detection of the beacon at the 4 cardinal positions.
	...
	case CENTRE_FROM_RIGHT: //This is if the robot is tweaking forward alignment having just turned from the right.
		...
		if(minLevelFrontSensor < returnAverageMin()) //Checks if the min level of the front sensor is below the accepted values.
		{
			if(globalMin < minLevelFrontSensor) //This checks to see if the sensor is reading things that are higher than the global min.
			{
				while(SensorValue[IRsensorFront] > (globalMin + 40)) //While the sensor is greater than the global min plus a deadband, turn towards it.
				{
	...
	case CENTER_FROM_LEFT: //This is if the robot is tweaking forward alignment having just turned from the left.
		...
		if(minLevelFrontSensor < returnAverageMin()) //Checks to see if the min level of the front sensor is below the accepted values.
		{
			if(globalMin < minLevelFrontSensor) //If this statement is true, the robot has turned past the beacon and needs to turn back.
			{
				while(SensorValue[IRsensorFront] > (globalMin + 40)) //While the sensor is greater than the global min plus some deadband, turn towards it.
				{
	...
	case BACK_TO_RIGHT: //This is if the robot is turning from the back sensor being detected to the right sensor being detected.
	...
	case LEFT_TO_FRONT: //This is if the robot is turning from the left sensor being detected to the front sensor being detected.
	...
	case RIGHT_TO_FRONT: //This is if the robot is turning from the right sensor being detected to the front sensor being detected.
	...
	case UNKNOWN:
	case BACK_TO_FRONT: //This is the default case, it will turn right if it doesn't have any detection or if the robot senses it at the back.
	...
	case CENTRED: //This case breaks out of the switch statement and in main calls the functions to commence the driving operation.
	default:
	}
...
task main() //The main task where all the other functions get called.
{
	...
	while(true)
	{
		...
		if(!test1ButtonPushed) //Checks to see if the button is pushed, just so it doesn't have to keep checking if it has been.
		{
